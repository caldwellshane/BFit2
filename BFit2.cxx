...
// err contains initial step sizes now, will contain error estimates later.
	...
	Int_t index;
	par[nCyc] = nCycles;
	// Avoid removable discontinuity in Z pops from gammaTi==gammaUi
	par[gammaU1] += 2*iota;
	par[gammaU2] += 2*iota;
	par[gammaU3] += 2*iota;
	// Parameter transformations to help fitting
	for (index = 0; index < nPars; index++) {
		//if (index == DC || index == r1 || index == r2 || index == r3) {
		if (index == r1 || index == r2 || index == r3) { // removed DC ... doesn't seem to work
			par[index] *= rateScale;
			err[index] *= rateScale;
		}
	...
	//	//	par[index] = 100.0 * par[index];
	//	//	par[index] = 1.0 - par[index];
	//		par[index] = 1.0 + Log(1.0+par[index]);
	//	}
	}
